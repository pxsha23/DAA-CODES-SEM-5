// A logistics company, SwiftCargo, specializes in delivering packages across multiple cities. To
// optimize its delivery process, the company divides the transportation network into multiple stages
// (warehouses, transit hubs, and final delivery points). Each package must follow the most
// cost-efficient or fastest route from the source to the destination while passing through these
// predefined stages. As a logistics optimization engineer, you must:
// 1. Model the transportation network as a directed, weighted multistage graph with multiple
// intermediate stages.
// 2. Implement an efficient algorithm (such as Dynamic Programming or Dijkstra’s Algorithm) to
...
// 4. Analyze and optimize route selection based on real-time constraints, such as traffic conditions,
// weather delays, or fuel efficiency.
        ...
        if (best_cost[src] >= INF / 2) {
            ...
            while (cur != -1) {
                ...
                if (nxt != -1) {
                    ...
                    bool found = false;
                    for (auto &e : adj[cur])
                        if (e.to == nxt) { ecost = e.cur_cost; found = true; break; }
                    if (!found) ecost = 0;
                    total += ecost;
                    cout << " -> ";
                } else break;
